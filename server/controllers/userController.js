...
const followUnFollowUser = async (req, res) => {
  try {
    const { id } = req.params;
    // Validate ObjectId to avoid CastError when invalid ids like "5" are passed
      ...
      try {
        if (user.profilePic) {
          try {
            ...
            console.error('Cloudinary delete error (non-fatal):', destroyErr);
        ...
        const errorMsg = cloudinaryErr.message || 'Cloudinary configuration error. Please check CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in your .env file.';
        return res.status(500).json({ 
          error: `Profile picture upload failed: ${errorMsg}`
        });
      }
    }
