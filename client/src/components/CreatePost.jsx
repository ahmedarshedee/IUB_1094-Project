...
const CreatePost = () => {
  ...
  const handleCreatePost = async () => {
    if (!postText.trim() && !imgUrl) {
      showToast('Error', 'Please add text or an image to your post', 'error');
      return;
    }
    ...
    try {
      ...
      const errorMessage = error?.message || error?.toString() || 'Failed to create post. Please try again.';
                    ...
                    if (selectedCategory) {
                      ...
                      try {
                        const generated = await generatePostWithGemini(
                          `Generate a social media post about ${selectedCategory}.`
                        );
                        setPostText(generated);
                        setRemainingChar(MAX_CHAR - generated.length);
                      } catch (err) {
                        showToast('Error', 'Failed to generate post', 'error');
                      } finally {
