// Using REST helper with env-based key instead of SDK
import { generatePostWithGemini } from './gemini.js';
...
const Actions = ({ post, isStatic }) => {
  ...
  async function handleReplyWithAI() {
    ...
    if (isGenerating) {
      showToast('Info', 'Please wait for the current reply to generate', 'info');
      return;
    }
    ...
    try {
      const prompt = 'Generate a short reply to a social media post with a maximum of 10 words. Keep it friendly and positive.';

      const text = await generatePostWithGemini(prompt);
      setReply((text || '').trim());
      showToast('Success', 'Reply generated! Press Reply to post it.', 'success');
    } catch (error) {
      console.error('Gemini error:', error);
      const msg = typeof error?.message === 'string' ? error.message : 'Failed to generate reply with Gemini API';
