...
// Global error handler for images to prevent console errors
window.addEventListener('error', (event) => {
  if (event.target && event.target.tagName === 'IMG') {
    // Suppress 404 errors for broken images
    if (event.message && event.message.includes('404')) {
      event.preventDefault();
      // Set src to empty to trigger Avatar fallback or hide image
      if (event.target.src) {
        event.target.src = '';
